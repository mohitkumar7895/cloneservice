...
export default function Sidebar() {
  ...
  const getLinkClass = (href: string, isSubItem = false) => {
    ...
    if (isSubItem) {
      return `group flex items-center space-x-3 px-10 py-2.5 text-[13px] font-medium transition-colors duration-150 rounded-lg mx-3 mb-0.5 ${
        active 
          ? "bg-emerald-500/10 text-emerald-300 border border-emerald-500/10 shadow-[0_0_10px_rgba(99,102,241,0.05)]" 
    ...
    return `group flex items-center justify-between px-4 py-3 mx-3 mb-1 text-[14px] font-medium transition-colors duration-150 rounded-xl ${
      active 
        ? "bg-gradient-to-r from-emerald-600/20 to-emerald-600/5 text-emerald-300 border border-emerald-500/20 shadow-[0_0_15px_rgba(99,102,241,0.1)] backdrop-blur-md" 
  ...
  const getIconClass = (href: string, isSubItem = false) => {
    ...
    return `transition-colors duration-150 ${
      active ? "text-emerald-400 drop-shadow-[0_0_8px_rgba(99,102,241,0.5)]" : "text-slate-500 group-hover:text-slate-300"
    }`;
  };
      ...
      <div className="h-16 px-6 bg-[#050B14]/80 backdrop-blur-xl border-b border-white/5 flex items-center justify-start sticky top-0 z-10 shrink-0">
        <div className="flex items-center gap-3">
          <div className="bg-white p-1 rounded-md shadow-sm">
            <img src="/logoprovisioningtech.webp" alt="PROVISIONINGTECH Logo" className="h-7 w-auto object-contain" />
          </div>
