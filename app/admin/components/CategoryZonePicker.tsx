...
export default function CategoryZonePicker({
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("provisioningtech_admin_zone_history");
      if (saved) {
        setZoneHistory(JSON.parse(saved));
      } else {
        const defaults = ["Noida", "Delhi", "Greater Noida", "Ghaziabad", "Bharthana", "Agra"];
        setZoneHistory(defaults);
        localStorage.setItem("provisioningtech_admin_zone_history", JSON.stringify(defaults));
      }
    } catch (e) {
  ...
  const saveHistory = (locationName: string) => {
    ...
    try {
      setZoneHistory((prev) => {
        ...
        localStorage.setItem("provisioningtech_admin_zone_history", JSON.stringify(updated));
        return updated;
      });
    } catch (e) {}
  };
  ...
  const clearHistory = () => {
    setZoneHistory([]);
    try {
      localStorage.removeItem("provisioningtech_admin_zone_history");
    } catch (e) {}
  };
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            <input
              ...
              className="w-full border border-gray-200 rounded-lg text-sm px-4 py-2.5 outline-none focus:border-emerald-500 bg-white pr-10 shadow-xs"
            />
            {inputValue && (
              <button
                type="button"
              ...
              <button
                ...
                className={`text-[11px] px-2.5 py-1 rounded-md font-medium transition ${
                  isAdded
                    ...
                    : "bg-gray-50 hover:bg-emerald-50 text-gray-700 hover:text-emerald-600 border border-gray-200 hover:border-emerald-200"
                }`}
              >
                + {preset}
              </button>
      ...
      <div className="space-y-1.5 pt-1">
        <div className="flex items-center justify-between text-xs font-bold text-gray-700">
          <span className="flex items-center gap-1.5">
            ...
            Live Map Area: <span className="text-emerald-600">{selectedMapLocation || "Delhi NCR"}</span>
          </span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
            {zoneHistory.map((item, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex items-center justify-between p-2 rounded-lg bg-white border border-gray-200 shadow-xs hover:border-emerald-300 transition"
                >
                  <button
                    type="button"
                    onClick={() => setSelectedMapLocation(item)}
                    className="flex items-center gap-1.5 text-xs font-semibold text-gray-800 hover:text-emerald-600 truncate flex-1 text-left"
                    title="View on Map"
                  >
