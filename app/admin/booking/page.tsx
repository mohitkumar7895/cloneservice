      ...
      <div className="bg-white/80 backdrop-blur-xl rounded-[24px] border border-white shadow-[0_8px_40px_rgb(0,0,0,0.04)] overflow-hidden">
        ...
        <div className="p-5 border-b border-gray-100/80 flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white/40">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            <ExportButtons tableId="bookingsTable" filename={`provisioningtech-bookings-${filter}`} />
            <div className="h-6 w-px bg-gray-300 hidden sm:block"></div>
            <div className="flex items-center gap-2 text-gray-600 font-medium">
              <span className="text-xs uppercase tracking-wider text-gray-500">Show</span>
              <select className="border border-gray-200 rounded-lg px-2 py-1.5 text-xs font-semibold outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 bg-white shadow-sm transition-all cursor-pointer">
                <option>50</option>
                <option>100</option>
              </select>
            </div>
        ...
        <div className="overflow-x-auto w-full">
          <table id="bookingsTable" className="w-full text-left border-collapse min-w-[1600px]">
            ...
            <tbody className="bg-white/20 divide-y divide-gray-50">
              {bookings.length > 0 ? (
                bookings.map((row) => (
                  <tr key={row.id} className="hover:bg-emerald-50/40 transition-colors duration-300 group">
                    ...
                    <td className="px-4 py-4 min-w-[100px] md:min-w-[150px] max-w-[130px] md:max-w-[200px] text-center">
                      {row.rating ? (
                        <div className="inline-flex flex-col items-center space-y-1">
                          ...
                          {row.review_tags && (
                            <div className="text-[10px] text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100 line-clamp-1">
                              {row.review_tags}
                            </div>
                          )}
                          {row.review && (
