...
export default async function WarrantiesPage({ searchParams }: { searchParams: Promise<{ filter?: string }> }) {
  ...
  try {
    const [allWarranties]: any = await pool.query(`
      SELECT w.id, w.customer_name, w.customer_phone, w.order_id, w.issued_date, w.expiry_date, w.days_valid, w.status,
             b.category as booking_category, b.type as booking_type, b.service_opted as cashback_opted
      FROM warranties w
              ...
              {warranties.length > 0 ? (
                warranties.map((row) => (
                  <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4">
                      <span className="font-semibold text-gray-800 text-xs bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-md border border-emerald-100">
