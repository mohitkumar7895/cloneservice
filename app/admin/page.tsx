...
export default async function AdminDashboard() {
  ...
  try {
    ...
    const [
      ...
      pool.query("SELECT id, order_id, customer_name, mobile, address, total, working_status, created_at FROM bookings ORDER BY created_at DESC LIMIT 5").catch(() => [[]]),
    ]);
  ...
  const managementSections = [
    {
      ...
      items: [
        { name: "Categories", desc: "Main service categories", href: "/admin/categories", icon: <ListTree className="w-5 h-5 text-emerald-500" />, bg: "bg-emerald-50" },
    ...
    },
    {
      ...
      items: [
        { name: "All Bookings", desc: "Master booking console", href: "/admin/booking", icon: <CalendarCheck className="w-5 h-5 text-emerald-500" />, bg: "bg-emerald-50" },
  ...
  return (
    <div className="font-sans relative pb-12 space-y-10 selection:bg-emerald-500/30">
      {/* Decorative Gradient Background */}
      <div className="absolute top-0 left-0 w-full h-[400px] overflow-hidden -z-10 pointer-events-none">
        <div className="absolute -top-32 -right-24 w-[500px] h-[500px] bg-emerald-400/10 rounded-full blur-[80px]"></div>
        <div className="absolute top-20 -left-32 w-[400px] h-[400px] bg-emerald-400/10 rounded-full blur-[80px]"></div>
      </div>

      {/* Hero Welcome Banner */}
      <div className="relative overflow-hidden rounded-[24px] bg-gradient-to-r from-emerald-950 via-slate-900 to-emerald-900 shadow-2xl p-6 sm:p-8 lg:p-10 text-white border border-emerald-800/50">
        <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-emerald-500/20 via-blue-500/10 to-transparent pointer-events-none"></div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/20 border border-emerald-400/30 mb-4 backdrop-blur-sm">
              ...
              <span className="text-[11px] font-bold text-emerald-300 uppercase tracking-wide">PROVISIONINGTECH Control Center Active</span>
            </div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-black tracking-tight mb-2">
              Welcome back, Admin! 👋
            </h1>
            <p className="text-emerald-200 text-sm md:text-base max-w-xl leading-relaxed">
          ...
          <div className="shrink-0 flex items-center gap-3">
            <Link href="/admin/booking">
              <button className="bg-emerald-500 hover:bg-emerald-400 text-white font-bold px-5 py-3 rounded-xl transition-all shadow-[0_0_20px_rgba(99,102,241,0.4)] flex items-center gap-2 text-sm cursor-pointer active:scale-95 border border-emerald-400/50">
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.06)] overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-gray-50/30">
          ...
          <Link href="/admin/booking" className="text-xs font-bold text-emerald-600 hover:text-emerald-800 flex items-center gap-1 transition">
                ...
                {recentBookings.map((b: any, idx: number) => (
                  <tr key={idx} className="hover:bg-gray-50/80 transition-colors">
                    <td className="px-5 py-4 font-bold text-gray-900">
                      <Link href="/admin/booking" className="hover:text-emerald-600">
                        #{b.id}
                      </Link>
                    </td>
                    <td className="px-5 py-4">
                      <p className="font-semibold text-gray-900">{b.customer_name || b.name || "Customer"}</p>
                    ...
                    <td className="px-5 py-4 font-bold text-gray-900">
                      ₹{Number(b.total ?? b.total_amount ?? 0).toLocaleString()}
                    </td>
                    <td className="px-5 py-4">
                ...
                <Link href={item.href} key={itemIdx}>
                  <div className="bg-white/80 backdrop-blur-lg p-4 sm:p-5 rounded-2xl border border-white shadow-[0_8px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_15px_30px_rgba(99,102,241,0.1)] hover:border-emerald-200/50 cursor-pointer transition-all duration-300 hover:-translate-y-1 group flex items-center gap-3.5 h-full relative overflow-hidden">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="text-[14px] font-bold text-gray-900 group-hover:text-emerald-600 transition-colors truncate">
                        {item.name}
                      </h4>
                      <p className="text-[12px] text-gray-400 truncate mt-0.5">{item.desc}</p>
                    </div>
                    <div className="relative shrink-0 w-7 h-7 rounded-full bg-gray-50 border border-gray-200/50 flex items-center justify-center text-gray-400 group-hover:bg-emerald-50 group-hover:text-emerald-600 transition-all duration-300 shadow-sm group-hover:shadow-md">
