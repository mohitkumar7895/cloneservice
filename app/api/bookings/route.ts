...
export async function POST(req: Request) {
  try {
    ...
    const token = cookieStore.get("provisioningtech_auth_token")?.value;
    if (token) {
      try {
...
export async function GET() {
  try {
    const [rows] = await pool.query(`SELECT id, order_id, customer_name, mobile, category, type, total, payment_status, working_status, created_at FROM bookings ORDER BY created_at DESC LIMIT 100`);
