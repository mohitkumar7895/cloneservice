...
export async function POST(req: Request) {
  try {
    ...
    if (action === 'claim_daily') {
      ...
      const userEmail = loggedEmail || booking.user_email || `${booking.mobile || 'user'}@provisioningtech.com`;
    ...
    if (bRows && bRows.length > 0) {
      const uEmail = loggedEmail || bRows[0].user_email || `${bRows[0].mobile || 'user'}@provisioningtech.com`;
