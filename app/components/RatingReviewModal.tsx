...
const RATING_EMOJIS = [
  ...
  { rating: 5, emoji: "🤩", label: "Outstanding!", color: "text-emerald-600 font-extrabold" },
];
...
export default function RatingReviewModal() {
  ...
  const checkPendingReview = async () => {
    try {
      ...
      const dismissedOrder = sessionStorage.getItem("provisioningtech_dismissed_rating_order");
  ...
  const handleDismiss = () => {
    if (booking) {
      sessionStorage.setItem("provisioningtech_dismissed_rating_order", booking.order_id);
    }
    setIsOpen(false);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        sessionStorage.removeItem("provisioningtech_dismissed_rating_order");
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-xs font-semibold cursor-pointer transition-all duration-200 border ${isSelected
                            ? "bg-gradient-to-r from-blue-600 to-emerald-600 text-white border-transparent shadow-sm scale-102"
