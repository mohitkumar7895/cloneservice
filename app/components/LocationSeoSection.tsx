      ...
      <div className="max-w-7xl mx-auto px-4 md:px-10 lg:px-12 py-6 md:py-8">
        <nav aria-label="Breadcrumb" className="text-xs sm:text-sm text-gray-500 mb-3">
          <ol className="flex flex-wrap items-center gap-1">
            ...
            <li>
              <Link href={`/${location.slug}`} className="hover:text-[#059669] font-medium">
                {location.title}
              </Link>
            </li>
            {service && (
        ...
        <div>
          ...
          <p className="mt-4 text-sm text-gray-700 max-w-3xl">
            ...
            and water purifier repair. Local technicians, 8 AM – 8 PM, 30-day warranty. Call 63976 50818.
          </p>
        </div>
          ...
          {(service ? SEO_SERVICES.filter((item) => item.slug === service.slug) : SEO_SERVICES).map((item) => (
            <div key={item.slug} className="rounded-2xl border border-gray-100 bg-slate-50 p-4">
              ...
              <Link
                ...
                className="inline-block mt-3 text-xs font-bold bg-[#059669] text-white px-3 py-1.5 rounded-lg"
              >
                View & Book
              </Link>
            </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <a
            ...
            className="text-xs sm:text-sm font-semibold bg-[#059669] text-white px-3 py-1.5 rounded-full"
          >
            Call 63976 50818
          </a>
          <Link
            href="/service-areas"
            className="text-xs sm:text-sm font-semibold bg-[#eef1fc] text-[#059669] px-3 py-1.5 rounded-full"
          >
            All service areas
          </Link>
        </div>
