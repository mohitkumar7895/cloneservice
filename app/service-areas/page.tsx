...
export const metadata: Metadata = {
  ...
  description:
    "All PROVISIONINGTECH Company service cities and societies: RO repair, refrigerator repair and washing machine repair in Noida, Gaur City 2, White Orchid, Flora, Mahagun Mywoods and more.",
    ...
    <main className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <div className="flex-grow max-w-5xl mx-auto w-full px-4 sm:px-6 py-10 sm:py-12">
        <nav className="text-xs sm:text-sm text-gray-500 mb-4">
          <Link href="/" className="hover:text-[#059669] font-medium">
            Home
          </Link>
            ...
            <ul className="flex flex-wrap gap-2">
              {SEO_KEYWORD_PAGES.map((kw) => (
                <li key={kw.slug}>
                  <Link
                    href={`/${kw.slug}`}
                    className="inline-block text-[11px] sm:text-xs font-semibold bg-[#eef1fc] text-[#059669] px-2.5 py-1.5 rounded-full hover:bg-[#059669] hover:text-white"
                  >
                    {kw.title}
                  </Link>
                </li>
  ...
  return (
    <section className="mt-8">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden divide-y divide-gray-50">
        {locations.map((loc) => (
          <div key={loc.slug} className="px-4 sm:px-6 py-4 hover:bg-emerald-50/40 transition-colors">
            <Link href={`/${loc.slug}`} className="block group">
              <p className="font-bold text-gray-900 group-hover:text-[#059669] text-base">
                {loc.title}
              </p>
            ...
            <div className="mt-2.5 flex flex-wrap gap-1.5">
              {SEO_SERVICES.map((svc) => (
                <Link
                  ...
                  className="text-[11px] sm:text-xs font-semibold bg-[#eef1fc] text-[#059669] px-2.5 py-1 rounded-full hover:bg-[#059669] hover:text-white"
                >
                  {svc.shortName}
                </Link>
              ))}
              {SEO_KEYWORD_PAGES.filter((kw) => keywordFitsLocation(kw, loc.slug)).map((kw) => (
                <Link
                  ...
                  className="text-[11px] sm:text-xs font-semibold bg-slate-100 text-slate-700 px-2.5 py-1 rounded-full hover:bg-[#059669] hover:text-white"
                >
                  {kw.title}
                </Link>
              ))}
