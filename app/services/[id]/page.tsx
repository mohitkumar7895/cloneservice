...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  if (!categoryId || isNaN(Number(categoryId))) {
    return { title: absoluteTitle("Service Category | PROVISIONINGTECH Company"), robots: { index: false, follow: true } };
  }

  try {
    const [catRows]: any = await pool.query("SELECT * FROM categories WHERE id = ?", [categoryId]);
    if (!catRows || catRows.length === 0) {
      return { title: absoluteTitle("Services | PROVISIONINGTECH Company"), robots: { index: false, follow: true } };
    }
    const cat = catRows[0];
    const catTitle = cat.title || "Appliance Service";

    let pageTitle = `${catTitle} in Delhi NCR | Doorstep Repair & Service`;
    let metaDescription = `Book expert doorstep ${catTitle} with PROVISIONINGTECH Company in Delhi NCR. Genuine parts, certified technicians, upfront rate card & 30-day warranty.`;
    if (cat.id === 1 || catTitle.toLowerCase().includes("ac repair")) {
      ...
      metaDescription = "RO repair near me, RO service near me and water purifier repair near me in Noida. Filter, TDS and leakage at home. Call 63976 50818.";
      ...
      metaDescription = "Refrigerator repair near me, fridge repair near me and refrigerator technician near me in Noida. Same-day cooling and gas service. Call 63976 50818.";
      ...
      metaDescription = "Washing machine repair near me, washing machine service near me and washing machine technician near me in Noida. Call 63976 50818.";
    }
    ...
    return {
      title: absoluteTitle("Services | PROVISIONINGTECH Company"),
    };
  }
}
