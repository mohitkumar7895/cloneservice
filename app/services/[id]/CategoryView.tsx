...
export default function CategoryView({ category, subcategories, services, rateCards = [] }: CategoryViewProps) {
  ...
  useEffect(() => {
    const savedCart = localStorage.getItem("provisioningtech_cart");
    if (savedCart) {
      try {
  ...
  const handleAddService = (service: any) => {
    ...
    setCart((prev) => {
      ...
      setTimeout(() => {
        localStorage.setItem('provisioningtech_cart', JSON.stringify(newCart));
    ...
    <div className="max-w-7xl mx-auto w-full bg-white min-h-screen pb-24">
        ...
        <div>
          <nav aria-label="Breadcrumb" className="text-xs text-gray-500 mb-2">
            <ol className="flex flex-wrap items-center gap-1">
              ...
              <li><Link href="/services" className="hover:text-[#059669]">Services</Link></li>
              <li aria-hidden="true">/</li>
          ...
          <p className="text-gray-500 text-xs sm:text-sm max-w-2xl">
            Book doorstep {(category.title || "appliance repair").toLowerCase()} with certified PROVISIONINGTECH technicians. Upfront rate card, genuine parts and 30-day service warranty across Noida, Delhi, Greater Noida, Ghaziabad and Gurgaon.
          </p>
        </div>
      </div>
                ...
                return (
                  <div 
                    ...
                    className={`flex flex-col items-center justify-start p-1.5 sm:p-2 rounded-xl cursor-pointer transition-all duration-200 group w-full ${
                      activeSubcat === sub.id 
                        ? 'bg-[#eef1fc] ring-2 ring-[#059669] shadow-xs' 
                  ...
                  >
                    ...
                    <span 
                      className={`text-[10px] sm:text-[11px] lg:text-[12px] text-center font-bold leading-tight break-words w-full px-0.5 ${
                        activeSubcat === sub.id ? 'text-[#059669]' : 'text-gray-700 group-hover:text-[#059669]'
                      }`}
        ...
        <div className="flex-1">
          {subcategories.map((sub) => {
            ...
            return (
              ...
              >
                <h2 className="text-[20px] lg:text-[22px] font-bold text-[#059669] mb-4 lg:mb-6 pb-2 border-b border-[#059669]">
                  {sub.title}
                </h2>
          ...
          <div className="hidden lg:block lg:w-[320px] flex-shrink-0">
            <div className="sticky top-[100px] bg-white border border-gray-100 rounded-2xl p-5 shadow-[0_8px_30px_rgba(0,0,0,0.08)]">
              ...
              <div className="max-h-[40vh] overflow-y-auto mb-4 pr-1">
                {cart.map((item) => (
                  <div key={item.id} className="flex justify-between items-start mb-4">
                    ...
                    {/* Quantity Selector */}
                    <div className="flex items-center justify-between bg-white border border-[#059669]/30 rounded-lg shadow-sm h-8 w-[72px] px-1 shrink-0">
                      <button onClick={() => handleRemoveService(item.id)} className="w-6 h-6 flex items-center justify-center text-[#059669] hover:bg-gray-50 rounded">
                        <span className="text-lg leading-none mt-[-2px]">-</span>
                      </button>
                      <span className="text-sm font-bold text-[#059669]">{item.quantity}</span>
                      <button onClick={() => handleAddService(item)} className="w-6 h-6 flex items-center justify-center text-[#059669] hover:bg-gray-50 rounded">
              ...
              <button 
                ...
                className="w-full bg-[#059669] hover:bg-[#047857] text-white font-bold py-3.5 rounded-xl transition shadow-md flex justify-between items-center px-4"
              >
      ...
      >
        <div 
          ...
          className="bg-[#059669] text-white rounded-2xl shadow-[0_8px_30px_rgba(96,105,201,0.4)] flex items-center justify-between p-4 cursor-pointer active:scale-[0.98] transition-transform"
        >
          ...
          >
              ...
              {cart.map((item) => (
                <div key={item.id} className="flex justify-between items-center py-5 border-b border-gray-50/50 last:border-0">
                  <div className="flex-1 pr-4">
                    ...
                    <div className="text-[15px] font-extrabold text-[#059669]">₹{Number(item.selling_price)}</div>
                  </div>
                  ...
                  <div className="flex items-center justify-between bg-white border border-gray-200 rounded-xl shadow-sm h-10 w-[90px] px-1.5 shrink-0">
                    <button onClick={() => handleRemoveService(item.id)} className="w-7 h-7 flex items-center justify-center text-[#059669] hover:bg-gray-50 rounded-lg active:bg-gray-100 transition-colors">
                    ...
                    <button onClick={() => handleAddService(item)} className="w-7 h-7 flex items-center justify-center text-[#059669] hover:bg-gray-50 rounded-lg active:bg-gray-100 transition-colors">
            ...
            <div className="p-5 border-t border-gray-100 bg-white/80 backdrop-blur-md pb-safe">
              <button 
                ...
                className="w-full bg-[#059669] hover:bg-[#047857] active:scale-[0.98] text-white font-bold py-4 rounded-2xl transition-all duration-300 shadow-[0_8px_20px_rgba(96,105,201,0.3)] flex justify-between items-center px-6"
              >
