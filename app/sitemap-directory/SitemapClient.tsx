...
export default function SitemapClient({ initialLinks }: { initialLinks: SitemapLink[] }) {
  ...
  return (
    <div className="space-y-8 sm:space-y-12 pb-16">
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-[#111936] via-[#1a2552] to-[#253270] text-white pt-8 pb-10 sm:pt-12 sm:pb-16 px-4 sm:px-6 lg:px-8 border-b border-emerald-500/20 shadow-2xl">
        {/* Glowing gradient circles */}
        <div className="absolute -top-24 -right-24 w-72 sm:w-96 h-72 sm:h-96 bg-emerald-500/15 rounded-full blur-3xl pointer-events-none" />
        ...
        <div className="max-w-7xl mx-auto relative z-10 space-y-5 sm:space-y-6">
          {/* Breadcrumb */}
          <nav className="flex items-center space-x-2 text-xs font-medium text-emerald-200/70">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-center">
            ...
            <div className="lg:col-span-8 space-y-4">
              ...
              <h1 className="text-2xl sm:text-4xl lg:text-5xl font-black tracking-tight leading-tight text-white drop-shadow-sm">
                PROVISIONINGTECH Company <span className="bg-gradient-to-r from-blue-200 via-emerald-200 to-amber-200 bg-clip-text text-transparent">Site Directory</span>
              </h1>
              ...
              <div className="pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-3 max-w-2xl">
                <div className="relative flex-1">
                  <Search className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-400 absolute left-4 top-1/2 -translate-y-1/2" />
                  <input
                    type="text"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    placeholder="Search service or locality (e.g. AC Repair, Sector 62)..."
                    className="w-full bg-white text-gray-900 placeholder-gray-400 text-xs sm:text-sm font-medium rounded-2xl pl-11 sm:pl-12 pr-10 py-3 sm:py-3.5 shadow-xl focus:outline-none focus:ring-4 focus:ring-emerald-400/30 transition"
                  />
                  {searchTerm && (
                    <button
            ...
            <div className="lg:col-span-4 bg-white/10 backdrop-blur-xl border border-white/15 rounded-3xl p-5 sm:p-6 space-y-3.5 shadow-2xl">
              ...
              <div className="flex items-center justify-between text-xs text-blue-100 pt-1">
                ...
                <span className="text-emerald-200 text-[11px] sm:text-xs">Delhi NCR Coverage</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 sm:space-y-10">
                ...
                <button
                  ...
                  className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-bold transition shrink-0 flex items-center gap-1.5 ${
                    activeTab === tab.id
                      ...
                      : "bg-slate-100 text-slate-700 hover:text-emerald-600 hover:bg-slate-200 border border-slate-200"
                  }`}
                >
                  <span>{tab.label}</span>
                  <span
        ...
        {searchTerm && (
          <div className="bg-emerald-50/80 border border-emerald-100 rounded-2xl p-3.5 sm:p-4 flex items-center justify-between">
            <span className="text-xs font-semibold text-emerald-900">
              Found <strong className="text-emerald-700">{filteredLinks.length}</strong> links matching &ldquo;{searchTerm}&rdquo;
            </span>
            <button
              onClick={() => {
                setSearchTerm("");
        ...
        {(activeTab === "All" || activeTab === "Services") && servicesLinks.length > 0 && (
          <section className="space-y-4 sm:space-y-5">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              ...
              <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2.5 sm:px-3 py-1 rounded-full border border-emerald-100 shrink-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
              {servicesLinks.map((link) => (
                <Link
                  key={link.id}
                  href={link.url}
                  className="group relative p-5 sm:p-6 bg-white rounded-3xl border border-slate-200/90 hover:border-emerald-500 shadow-2xs hover:shadow-xl hover:shadow-emerald-500/10 transition-all duration-300 flex flex-col justify-between overflow-hidden"
                >
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="p-2 rounded-xl bg-slate-50 group-hover:bg-emerald-50 text-slate-400 group-hover:text-emerald-600 transition">
                        <ArrowUpRight className="w-4 h-4" />
                      </div>
                    </div>

                    <h3 className="font-extrabold text-slate-900 group-hover:text-emerald-600 transition text-sm sm:text-base leading-snug">
                  ...
                  <div className="mt-4 sm:mt-5 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                    <span className="font-extrabold text-emerald-700 group-hover:underline flex items-center gap-1">
        ...
        <div className="bg-gradient-to-r from-blue-900 via-emerald-900 to-purple-900 text-white rounded-3xl p-6 sm:p-10 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
